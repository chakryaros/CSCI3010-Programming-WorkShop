...
function git_indicator {
    ...
    # hearts content 
    if [[ $git_status =~ "clean" ]] && [[ $git_status =~ "up to date" ]]; then
        echo -ne " color clean" # echo out something to indicate the state that you just tested for
    elif [[ ! $git_status =~ "do something" ]]; then
        echo -ne "not color"
    else
        echo -ne "either clean or dirty"
    fi
}
