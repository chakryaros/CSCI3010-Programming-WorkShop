...
class Election {
public:
    Election(vetor<Candidate> temp_candidate); //refarctory constructor
    void Display_electoralMap(); //display district, square mile and each party
    
    void campaigning(int district_id, Party party); //calculate the convert a voter to constituent
    void printCandidate(); //display each Candidate name, ids and party
    void get_Election_vote(); //ask user to get direct vote
    void Calculate_result(map<int, District *> district);
    void get_representative_vote();
private:
