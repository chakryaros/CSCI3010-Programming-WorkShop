...
int main() {
  ...
  server.resource["^/chatrooms$"]["GET"] = [](shared_ptr<HttpServer::Response> response, shared_ptr<HttpServer::Request> request) {
    ...
    for(auto &field : query_fields) {
      ...
      // ADDED: headers to tell the browser how to render it!
      stringstream header_stream;
      header_stream << "Content-Type: text/html";
      SimpleWeb::CaseInsensitiveMultimap headers = SimpleWeb::HttpHeader::parse(header_stream);
      ...
      *response << "HTTP/1.1 400 Bad Request\r\nContent-Length: " << strlen(e.what()) << "\r\n\r\n"
      ...
      server.resource["^/announce$"]["POST"] = [](shared_ptr<HttpServer::Response> response, shared_ptr<HttpServer::Request> request) {
          ...
          try {
              ...
              string name = pt.get<string>("name");
              string room = pt.get<string>("room");
              string announce = pt.get<string>("ANNOUNCEMENT");
              //cout << "Adding announce to room: " << room << endl;
              //display message in all chatroom
              for(int i=0; i<room.size(); i++)
              {
                  cout<<LoadChatLog(room[i])<<endl;
              }
              ...
              ofstream rooms_outfile;
              rooms_outfile.open(room + ".txt", std::ios_base::app);
              rooms_outfile << name << ": " << announce << endl;
              rooms_outfile.close();
